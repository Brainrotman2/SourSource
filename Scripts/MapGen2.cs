...
public class MapGen2 : MonoBehaviour
{
    ...
    void PlaceWorldEventToken()
    {
        ...
        worldEventTokenInstance = Instantiate(worldEventTokenPrefab, pos, Quaternion.identity, transform);
        currentRing = 0;
        currentRingIndex = 0;
    }
    ...
    void PlacePlayers()
    {
        ...
        for (int i = 0; i < numberOfPlayers; i++)
        {
            ...
            if (hexTiles.ContainsKey(startHex))
            {
                ...
                GameObject marker = Instantiate(playerMarkerPrefab, world + Vector3.up * 0.1f, Quaternion.identity, transform);
